// problem name: Intersention of Cubes
// problem link: http://lightoj.com/volume_showproblem.php?problem=1211
// contest link: (?)
// time: (?)
// author: reyad
...
//            for(int i=0; i<n; i++) area += buf[i].x * buf[i+1].y - buf[i+1].x * buf[i].y;
    ...
    static class CustomPoint implements Comparable<CustomPoint> {
        long x, y, z;
        CustomPoint(long x, long y, long z) {
            this.x = x;
            this.y = y;
            this.z = z;
        }
        public int compareTo(CustomPoint p) {
            if(this.x == p.x) {
                if(this.y == p.y) {
    ...
    static void solve() throws IOException {
        try {
            ...
            for(int cc=0; cc<tc; cc++) {
                int n = in.nextInt();
                int m = 2 * n;
                CustomPoint[] p = new CustomPoint[m];
                int[] x = new int[m];
                int[] y = new int[m];
                int[] z = new int[m];
                for(int i=0; i<m; i++) {
                    ...
                    y[i] = in.nextInt();
                    z[i] = in.nextInt();
                    p[i] = new CustomPoint(x[i], y[i], z[i]);
                }
                Arrays.sort(x);
                Arrays.sort(y);
                Arrays.sort(z);
                ...
                for(i=0; i<m; i+=2) {
                    if(
                        inside(p[i].x, p[i+1].x, x[n]) && inside(p[i].x, p[i+1].x, x[n-1])
                        && inside(p[i].y, p[i+1].y, y[n]) && inside(p[i].y, p[i+1].y, y[n-1])
                        && inside(p[i].z, p[i+1].z, z[n]) && inside(p[i].z, p[i+1].z, z[n-1])
                        ) {
                        continue;
                    }
                    break;
                ...
                int vol = 0;
                if(i == m) vol = Math.abs((x[n] - x[n-1]) * (y[n] - y[n-1]) * (z[n] - z[n-1]));
                out.println("Case " + (cc+1) + ": " + vol);
            }
        }
        catch(IOException e) {
            // expected
